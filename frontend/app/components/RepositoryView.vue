...
<template>
  <div>
    ...
    <div class="flex items-center gap-2 mb-6">
      <UIcon name="i-heroicons-book-open" class="w-6 h-6 text-zinc-400" />
      <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">{{ repo.name }}</h1>
      <UBadge v-if="repo.is_private" color="neutral">Private</UBadge>
    </div>
    ...
    <div v-if="viewType === 'tree'" class="space-y-6">
      <UCard class="p-0 overflow-hidden">
        <div class="border-b border-zinc-100 dark:border-zinc-800 p-3 bg-zinc-50 dark:bg-zinc-900/50 flex justify-between">
          <div class="text-sm text-zinc-500 font-mono">latest commit</div>
        </div>
        ...
        <div v-else class="divide-y divide-zinc-100 dark:divide-zinc-800">
          ...
          <div v-if="!files || files.length === 0" class="p-8 text-center text-zinc-400 text-sm">Empty repository.</div>
        </div>
      </UCard>
    ...
    <!-- VIEW MODE: BLOB -->
    <UCard v-else-if="viewType === 'blob'" class="overflow-hidden p-0">
