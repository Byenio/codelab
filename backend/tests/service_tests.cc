...
TEST_F(ServiceTest, CreateRepoSuccess) {
  ...
  auto repo = service.CreateRepository(uid, dirId, "MyRepo", "Desc", true);
...
TEST_F(ServiceTest, DuplicateNameFails) {
  ...
  auto r1 = service.CreateRepository(uid, std::nullopt, "SameName", "", true);
  ASSERT_TRUE(r1.has_value());

  auto r2 = service.CreateRepository(uid, std::nullopt, "SameName", "", true);
